class SharedRecordsController < ApplicationController
  ...
  def show
    @vaccinations = @cr_access_data.vaccination_records_accessor(reload: true)
  end

  private

  def set_cr_access_data
    @cr_access_group = current_user.accepted_cr_groups.find_by(id: params[:id]) || current_user.owned_cr_groups.find_by(id: params[:id])
